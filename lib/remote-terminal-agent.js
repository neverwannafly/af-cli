...
class RemoteTerminalAgent {
  constructor({ connectUrl, agentToken, slug, cwd, onClose, onTerminalExit }) {
  ...
  start() {
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.once('open', () => {
        ...
        // The current gateway does not ACK hello; preserve the TunnelAgent's
        // startup behaviour while allowing a future explicit ready message.
        setTimeout(() => {
  ...
  attach(attachmentId, cols, rows) {
    ...
    if (this.attachmentId && this.attachmentId !== attachmentId) {
      sendJson(this.socket, {
    ...
    if (this.scrollback.length > 0) {
      // Mark this as replayed history. The scrollback may contain terminal
      // capability queries (DA, DSR/CPR, DECRQM, OSC color) the shell emitted
      // before any browser was attached. A freshly opened browser terminal must
      // not answer them, or the belated replies arrive as input at the idle
      // prompt and get echoed/executed as garbage commands.
      sendJson(this.socket, {
        type: 'terminal_output',
  ...
  detach(reason) {
    ...
    sendJson(this.socket, { type: 'terminal_detached', attachment_id: this.attachmentId, reason });
    this.attachmentId = null;
  }
...
module.exports = { RemoteTerminalAgent, resolveShell };
