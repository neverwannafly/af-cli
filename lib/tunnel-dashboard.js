...
class TunnelDashboard {
  ...
  render() {
    if (!process.stdout.isTTY) {
      ...
      for (const log of rows) console.log(this.formatLogLine(log));
    ...
    console.log(chalk.bold('API Frenzy Tunnel'));
    console.log(`${chalk.gray('Public')}  ${this.publicUrl || '-'}`);
    console.log(`${chalk.gray('Local')}   http://127.0.0.1:${this.localPort}`);
    console.log(`${chalk.gray('Uptime')}  ${stats.uptimeSeconds}s`);
    console.log('');
    console.log([
    ...
    console.log('');
    console.log(`${chalk.gray('Throughput')} [${chalk.cyan(throughput.graph)}] ${chalk.gray('last 30s, 2s buckets, peak')} ${throughput.peak.toFixed(1)} req/s`);
    console.log('');
    console.log(chalk.gray('Press Ctrl+C to stop. Showing last 100 in-memory events.'));
    // Durable analytics belong in backend infra later: ClickHouse, SigNoz, or a similar metrics/log pipeline.
    console.log('');
    console.log(chalk.gray(`${pad('TIME', 8)} ${pad('TYPE', 5)} ${pad('METHOD', 7)} ${pad('PATH', 34)} ${right('STATUS', 6)} ${right('DURATION', 9)} ${right('IN', 9)} ${right('OUT', 9)}  NOTE`));
    console.log(chalk.gray('-'.repeat(112)));
